            ...
            <div style="position: relative;">
                ...
                <select id="region_id" name="region_id" style="display: none;">
                    ...
                    @foreach($regions as $region)
                        <option value="{{ $region->id }}" data-region="{{ $region->region }}" data-city="{{ $region->city ?? '' }}" {{ (old('region_id', $trackerInfo->region_id) == $region->id) ? 'selected' : '' }}>
                            @if($region->city)
...
<script>
    ...
    // Set initial value
    @php
        $selectedRegionId = old('region_id', $trackerInfo->region_id);
        if ($selectedRegionId) {
            $selectedRegion = \App\Models\Region::find($selectedRegionId);
            if ($selectedRegion) {
                $regionDisplay = $selectedRegion->city ? $selectedRegion->city . ', ' . $selectedRegion->region : $selectedRegion->region;
            }
        }
    @endphp
    @if(isset($selectedRegionId) && $selectedRegionId)
        const selectedRegion = regionSelect.querySelector('option[value="{{ $selectedRegionId }}"]');
        if (selectedRegion) {
